...
import { toAPY, toPrecision } from '@/utils/saber/number';
import { SBR_INFO } from '@/utils/saber/builtinTokens';
import { getLogo, getPoolId, getPoolName, getSymbol } from '@/utils/saber/pool';

import useClaim from '@/hooks/saber/user/useClaim';
import usePoolsInfo from '@/hooks/saber/usePoolsInfo';
...
import useClaimableRewards from '@/hooks/saber/user/useClaimableRewards';
...
import useDailyRewards from '@/hooks/saber/user/useDailyRewards';
...
import Tabs from '@/components/saber/ui/Tabs';
import { StakeForm } from '@/components/saber/pool/StakeForm';
import { WithdrawForm } from '@/components/saber/pool/WithdrawForm';
import { UnstakeForm } from '@/components/saber/pool/UnstakeForm';
import { DepositForm } from '@/components/saber/pool/DepositForm';
import { PopoverRef, UniversalPopover } from '@/components/saber/modals/UniversalPopover';
import { TokenDisplay, TokenLogo } from '@/components/saber/shared/TokenDisplay';
import { useUpgradeStake } from '@/hooks/saber/user/useUpgradeStake';
    ...
    return (
        <Block className="w-full h-full">
            ...
            <div className="flex items-center gap-1">
                ...
                {/* @ts-expect-error - other is not typed in TokenInfo extensions */}
...
const LiquidityBlock = (props: { pool: PoolData; handleOpenModel?: () => void }) => {
    ...
    }, [miner, props, maxSlippagePercent]);
                    ...
                    data={[
                        [
                            ...
                            <div key={`${token0.address}-deposits`} className="flex items-center gap-1">
                                <Image 
                                    src={getLogo(token0.symbol, token0.logoURI)} 
                                    alt={`${getSymbol(token0.symbol)} logo`}
                                    width={20}
                                    height={20}
                                    className='w-5 h-5'
                                />
                        ...
                        [
                            ...
                            <div key={`${token1.address}-deposits`} className="flex items-center gap-1">
                                <Image 
                                    src={getLogo(token1.symbol, token1.logoURI)} 
                                    alt={`${getSymbol(token1.symbol)} logo`}
                                    width={20}
                                    height={20}
                                    className='w-5 h-5'
                                />
...
export default function PoolPage({ params }: { params: { id: string } }) {
    ...
    const poolData = [
        ['---'],
        ['SBR emission rate', pool ? <EmissionRate key="emission-rate" pool={pool} /> : null],
        ...(pool.replicaQuarryData ? pool.replicaQuarryData.map((replica, index) => {
            if (replica.data.annualRewardsRate.toString() === '0') {
                return [];
            }
            return [
                <div key={`token-display-${index}`}>
                    <TokenDisplay 
                        name={replica.info.redeemer?.tokenInfo.symbol} 
                        mint={replica.info.rewardsToken.mint} 
                    /> emission rate
                </div>, 
                <ReplicaEmissionRate key={`replica-emission-${index}`} replica={replica} />,
            ];
        }) : []),
        ['---'],
        [
                                            ...
                                            <div className="flex items-center gap-1 justify-start sm:justify-end">
                                                ...
                                                <div className="flex gap-2">
                                                    <Image
                                                        ...
                                                        alt={`${getSymbol(pool.info.tokens[0].symbol)} logo`}
                                                        width={20}
                                                        height={20}
                                                        className='w-5 h-5'
                                                    />
                                                    <Image
                                                        ...
                                                        alt={`${getSymbol(pool.info.tokens[1].symbol)} logo`}
                                                        width={20}
                                                        height={20}
                                                        className='w-5 h-5'
                                                    />
