...
import { valuesToKeys } from '@/utils/saber/keys';
import { parseRawSwapState } from '@/utils/saber/state';
import { DetailedSwapSummary, PoolData, PoolInfo, PoolInfoRaw } from '@/types/saber';
import { getExchange } from '@/utils/saber/exchange';
import {
    getPoolTVL,
    getPoolTokenPercentages,
    getPrice,
} from '@/utils/saber/prices';
import {
    getEmissionApy,
    getFeeApy,
    getSecondaryEmissionApy,
} from '@/utils/saber/apy';
import { findMergePoolAddress } from '@/utils/saber/replicaRewards';
import { getRewarder, QuarryRewarderInfo } from '@/utils/saber/rewarder';
...
const cachedJupTokenInfo: Record<string, JupiterTokenInfo> = {
...
const calculateUsdValue = (
    ...
    const usdValue =
        valueA * pool.usdPrice.tokenA + valueB * pool.usdPrice.tokenB;
...
const getQuarryInfo = async (
    quarry: QuarrySDK,
    rewarders: QuarryRewarderInfo[],
    pools: PoolData[]
) => {
    // Get quarry info
        ...
        await Promise.all(
            chunks.map(
                throat(10, async (chunk) => {
                    ...
                    >[] = (await quarry.programs.Mine.account.quarry.fetchMultiple(
                        chunk
                    )) as any;
                    return result;
                })
    ...
    pools.forEach((pool) => {
        ...
        const replicaInfo =
            mergePoolAddress &&
            rewarders?.find((rewarder) => rewarder.mergePool === mergePoolAddress);
        if (!replicaInfo) {
            return;
        }
        ...
        pool.replicaQuarryData = replicaInfos
            ...
            .map((data) => ({
                data,
                info: replicaInfo.replicaQuarries.filter(
                    (replica) => replica.rewarder.toString() === data.rewarder.toString()
                )?.[0],
            }));
    });
};
...
export default function usePoolsInfo() {
    ...
    return useQuery({
        queryKey: [
            'registryPoolsInfo',
            endpoint,
            wallet?.adapter.publicKey ? 'connected' : 'disconnected',
        ],
        queryFn: async () => {
            ...
            await Promise.all(
                data.pools.map(async (pool) => {
                    if (pool.replicaQuarryData) {
                        if (pool.info.name === 'SOL-bSOL') console.log(pool);
                        await Promise.all(
                            pool.replicaQuarryData.map(async (replica) => {
                                if (replica.data.annualRewardsRate.lte(new BN(0))) {
                                    return 0;
                                }

                                // Check if there is a redeemer for this token
                                const rewarder = await getRewarder(
                                    network,
                                    replica.info.rewarder
                                );
                                const redeemer = rewarder.info.redeemer;
                                if (redeemer) {
                                    // Set up the redeemer token information
                                    let tokenInfo =
                                        cachedJupTokenInfo[redeemer.underlyingToken] ?? null;
                                    if (!tokenInfo) {
                                        try {
                                            tokenInfo = await (
                                                await fetch(
                                                    `https://tokens.jup.ag/token/${redeemer.underlyingToken}`
                                                )
                                            ).json();
                                        } catch (e) {
                                            tokenInfo = null;
            ...
            await Promise.all(
                data.pools.map(async (pool) => {
                    const metricInfo = poolsInfo[
                        pool.info.swap.config.swapAccount.toString()
                    ] ?? { v: 0, feesUsd: 0 };
                    ...
                    if (pool.replicaQuarryData) {
                        secondaryApy = await Promise.all(
                            pool.replicaQuarryData.map(async (replica) => {
                                if (replica.data.annualRewardsRate.lte(new BN(0))) {
                                    return 0;
                                }
                                ...
                                const price = await getPrice(
                                    replica.info.redeemer?.tokenInfo.address ??
                                        replica.info.rewardsToken.mint,
                    ...
                    const emissionApy = getEmissionApy(
                        pool,
                        prices[SBR_ADDRESS.toString()]
                    );
                    const tokenPercentages = getPoolTokenPercentages(pool);
                    const stakePoolApyToken0 =
                        100 *
                        (stakePoolApy[pool.info.swap.state.tokenA.mint.toString()] ?? 0) *
                        tokenPercentages.tokenA;
                    const stakePoolApyToken1 =
                        100 *
                        (stakePoolApy[pool.info.swap.state.tokenB.mint.toString()] ?? 0) *
                        tokenPercentages.tokenB;
                    pool.metrics = {
                        tvl,
                        feeApy,
                        emissionApy,
                        secondaryApy:
                            pool.replicaQuarryData?.map((_, i) => secondaryApy[i] ?? 0) ?? [],
                        stakePoolApyToken0,
                        stakePoolApyToken1,
                        totalApy:
                            feeApy +
                            emissionApy +
                            secondaryApy.reduce((acc, val) => acc + val, 0) +
                            stakePoolApyToken0 +
                            stakePoolApyToken1,
                    };
                })
            ...
            if (wallet?.adapter.publicKey) {
                ...
                const mergeMinerBalances = await Promise.all(
                    data.pools.map(async (pool) => {
                        invariant(wallet.adapter.publicKey);
                ...
                const chunks = chunk(
                    lpTokenBalances
                        .concat(legacyMinerBalances)
                        .concat(mergeMinerBalances),
                    100
                );
                ...
                tokenAmounts.map((amount) => {
                    if (amount.amount) {
                        ...
                        if (pool) {
                            ...
                            pool.userInfo.stakedBalance = new BN(pool.userInfo.stakedBalance)
                                .add(new BN(amount.amount))
                                .toString();
                            pool.userInfo.stakedUsdValue = calculateUsdValue(
                                pool,
                                pool.userInfo.stakedBalance.toString(),
                                maxSlippagePercent
                            );
                        }
                    }
