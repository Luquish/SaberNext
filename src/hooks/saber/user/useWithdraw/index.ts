    ...
    const handleWithdraw = async () => {
        ...
        // Merge miner withdraw IXs
            ...
            if (miner.mergeMiner && miner.replicaInfo) {
                ...
                const withdrawSbr = await mergePool.withdraw({
                    amount,
                    rewarder: SBR_REWARDER,
                    mergeMiner: mmAddress,
                });
            ...
            if (miner.stakedBalanceLegacy.gt(new BN(0))) {
                const amountLeftToUnstake = new BigNumber(
                    withdrawPoolTokenAmount.raw.toString()
                ).minus(amountUnstakedFromMM.toNumber());

                const maxAmount = BigNumber.min(
                    new BigNumber(miner.stakedBalanceLegacy.toString()),
                    amountLeftToUnstake
                );
                if (maxAmount.gt(0)) {
                    const amount = new TokenAmount(
            ...
            const claimTxs = await getClaimIxs(
                saber,
                quarry.sdk,
                miner,
                pool,
