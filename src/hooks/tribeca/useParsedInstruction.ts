...
import type { IdlAccount, IdlAccountItem } from '@project-serum/anchor/dist/cjs/idl'
...
/**
 * Flattens nested IDL accounts into a single array with prefixed names
 */
...
export type InstructionData =
    ...
    | { type: 'anchor'; args: { name: string; type: string; data: string }[] }
    | { type: 'object'; args: unknown }
...
/**
 * Hook to parse proposal instructions
 */
export function useParsedProposalInstruction(
...
/**
 * Hook to parse transaction instructions with Anchor or fallback parsing
 */
export function useParsedInstruction(
    ...
    const anchorIX = useMemo((): ParsedInstruction | null => {
        ...
        if (idlIx) {
            ...
            const accounts = ix.keys.map((meta, idx) => ({
                name: idx < flatIdlAccounts.length
                    ? flatIdlAccounts[idx]?.name ?? `Account #${idx + 1}`
                    : `Account #${idx + 1}`,
                ...meta,
            }))
            return {
    ...
    const decodedIX = useMemo((): ParsedInstruction | null => {
        if (anchorIX) return anchorIX

        const parsedNonAnchor = parseNonAnchorInstruction(ix)
        if (!parsedNonAnchor || 'error' in parsedNonAnchor) return null
        ...
        return {
            ...
            accounts: ix.keys.map((accountMeta, i): InstructionAccount => ({
                ...accountMeta,
                name: accountLabels?.[i] ?? `Account #${i + 1}`,
            })),
        }
    }, [anchorIX, ix])
