...
interface SmartWalletHookResult {
    key: PublicKey
...
/**
 * Internal hook for smart wallet functionality
 */
function useSmartWalletInner(key?: PublicKey): SmartWalletHookResult {
    if (!key) {
    ...
    const [smartWallet, setSmartWallet] = useState<SmartWalletWrapper | null>(null)
    const [txAddrs, setTxAddrs] = useState<PublicKey[]>([])
    const { data: txs } = useBatchedGokiTransactions(txAddrs)

    // Handle transaction addresses
    useEffect(() => {
        if (!smartWalletData) {
            setTxAddrs([])
            return
...
const { useContainer: useSmartWallet, Provider: SmartWalletProvider } =
    createContainer(useSmartWalletInner)

export { useSmartWallet, SmartWalletProvider }