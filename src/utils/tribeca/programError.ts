...
/**
 * Error thrown when IDL parsing fails
 */
...
/**
 * Parses IDL errors into a map of error codes to error representations
 */
...
/**
 * Error class for program-specific errors
 */
 ...
 */
const codesToName: Record<number, string> = Object.entries(LangErrorCode).reduce(
    (acc, [name, code]) => ({
        ...acc,
        [code]: name,
    }),
...
/**
 * Maps error codes to their messages
 */
const LangErrorMessage = new Map<number, string>([
    // Instructions
...
/**
 * Program-specific error handlers
 */
