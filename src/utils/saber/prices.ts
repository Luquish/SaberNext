...
export const getPoolTVL = (pool: Omit<PoolData, 'metrics'>) => {
    ...
    const tvl =
        amountTokenA.asNumber * usdPriceA + amountTokenB.asNumber * usdPriceB;
    return tvl;
};
...
export const getPrice = async (mint: string, decimals: number) => {
    ...
    try {
        const result = await fetch(`https://api.jup.ag/price/v2?ids=${mint}`).then(
            (res) => res.json()
        );
        if (!result.data) {
            throw Error('No data');
        ...
        Object.values(result.data).forEach((priceRecord: any) => {
            priceCache[priceRecord.id] = parseFloat(priceRecord.price);
        });
    } catch (e) {
        // Try using quote
        try {
            const quote = await fetch(
                `https://quote-api.jup.ag/v6/quote?inputMint=${mint}&outputMint=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v&amount=${
                    10 ** decimals
                }`
            ).then((res) => res.json());
