...
const getStakedTVL = (pool: PoolData) => {
    ...
    return valueA * pool.usdPrice.tokenA + valueB * pool.usdPrice.tokenB;
};
...
export const getSecondaryEmissionApy = (
    pool: PoolData, 
    replica: NonNullable<PoolData['replicaQuarryData']>[number], 
    price: number
) => {
